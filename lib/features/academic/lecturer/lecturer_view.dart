...
class _LecturerViewState extends State<LecturerView> {
  ...
  return Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      _buildSearchBar(), // Tetap tampil di halaman utama
      const SizedBox(height: 25),
      _buildStatCards(),
      ...
      FutureBuilder<List<ProjectModel>>(
        ...
        builder: (context, snapshot) {
          ...
          // LOGIKA FILTER: Hanya cari berdasarkan Judul Tugas Besar
          final filteredProjects = snapshot.data!.where((project) {
            return project.title.toLowerCase().contains(_searchQuery);
          }).toList();

          if (filteredProjects.isEmpty) {
            return const Center(child: Text("Tugas besar tidak ditemukan."));
          }
  ...
  // Hapus _buildSearchBar() dari sini
  return FutureBuilder<List<WorkspaceModel>>(
    ...
    builder: (context, snapshot) {
      ...
      // Tampilkan semua kelompok tanpa filter pencarian
      return ListView.builder(
        shrinkWrap: true,
        physics: const NeverScrollableScrollPhysics(),
  ...
  return Container(
    ...
    child: TextField(
      ...
      decoration: const InputDecoration(
        ...
        hintText: "Cari Tugas Besar...", // Sesuaikan teks hint
        ...
        child: Column(
          ...
          children: [
            Text(
              label,
              style: TextStyle(
                color: isBordered ? Colors.grey[600] : Colors.white,
                fontSize: 10,
                fontWeight: FontWeight.bold,
              ),
            ),
