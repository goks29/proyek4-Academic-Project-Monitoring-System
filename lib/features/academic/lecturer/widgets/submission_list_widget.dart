...
import '../../../../controllers/lecturer/submission_review_controller.dart'; // <--- IMPORT CONTROLLER
...
class _SubmissionListWidgetState extends State<SubmissionListWidget> {
  ...
  Future<void> _openLink(BuildContext context, String urlString) async {
    ...
    try {
      ...
      if (context.mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text("Gagal membuka tautan: $urlString")));
    }
  }
  ...
  // ... (Sisanya _buildStatusBadge dan build() BIAKAN SAMA PERSIS seperti sebelumnya)
  @override
  Widget build(BuildContext context) {
    if (widget.submissions.isEmpty) return const Text("Belum ada pengumpulan.", style: TextStyle(color: Colors.grey, fontStyle: FontStyle.italic));
    
    return ListView.separated(
      shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: widget.submissions.length,
                ...
                children: [
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Row(children: [Icon(Icons.upload_file, color: Colors.indigo.shade400, size: 20), const SizedBox(width: 8), Text("Pengumpulan #${index + 1}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 14))]),
                  ...
                  if (isPending)
                    if (isLoading) const SizedBox(height: 30, width: 30, child: CircularProgressIndicator(strokeWidth: 2))
                    else Row(
                      mainAxisSize: MainAxisSize.min,
                      children: [
                        OutlinedButton(onPressed: () => _handleReview(context, sub, false), style: OutlinedButton.styleFrom(foregroundColor: Colors.red.shade600, side: BorderSide(color: Colors.red.shade200), padding: const EdgeInsets.symmetric(horizontal: 10), minimumSize: const Size(0, 32)), child: const Text("REVISI", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold))),
                        const SizedBox(width: 6),
                        ElevatedButton(onPressed: () => _handleReview(context, sub, true), style: ElevatedButton.styleFrom(backgroundColor: Colors.green.shade600, foregroundColor: Colors.white, elevation: 0, padding: const EdgeInsets.symmetric(horizontal: 10), minimumSize: const Size(0, 32)), child: const Text("TERIMA", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold))),
                      ],
                    )
                  else _buildStatusBadge(sub.status),
                ],
              ),
              const SizedBox(height: 12),
              if (sub.studentNotes != null && sub.studentNotes!.isNotEmpty) ...[Text('"${sub.studentNotes!}"', style: TextStyle(color: Colors.grey.shade700, fontStyle: FontStyle.italic, fontSize: 13)), const SizedBox(height: 12)],
              if (hasFile) InkWell(onTap: () => _openLink(context, sub.evidenceFileUrl!), child: Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 8), decoration: BoxDecoration(color: Colors.blue.shade50, borderRadius: BorderRadius.circular(6), border: Border.all(color: Colors.blue.shade100)), child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.open_in_new, size: 14, color: Colors.blue.shade700), const SizedBox(width: 6), Text("Buka Tautan", style: TextStyle(color: Colors.blue.shade800, fontWeight: FontWeight.bold, fontSize: 12))]))),
              if (!isPending && sub.lecturerFeedback != null && sub.lecturerFeedback!.isNotEmpty) ...[
                const SizedBox(height: 16),
                Container(width: double.infinity, padding: const EdgeInsets.all(10), decoration: BoxDecoration(color: Colors.grey.shade50, borderRadius: BorderRadius.circular(8), border: Border.all(color: Colors.grey.shade200)), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text("Catatan Anda:", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: Colors.grey.shade500)), const SizedBox(height: 4), Text(sub.lecturerFeedback!, style: const TextStyle(fontSize: 13, color: Colors.black87))])),
              ],
            ],
          ),
        );
  ...
  Widget _buildStatusBadge(String status) {
    return Container(
      ...
      decoration: BoxDecoration(color: status.toLowerCase() == 'accepted' ? Colors.green.shade50 : Colors.red.shade50, borderRadius: BorderRadius.circular(4)),
      child: Text(status.toLowerCase() == 'accepted' ? "DITERIMA" : "REVISI", style: TextStyle(color: status.toLowerCase() == 'accepted' ? Colors.green.shade700 : Colors.red.shade700, fontSize: 10, fontWeight: FontWeight.bold)),
    );
  }
}